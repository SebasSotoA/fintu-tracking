...
export function PerformanceMetrics({ trades, cashFlows, fxRates, marketPrices }: PerformanceMetricsProps) {
  ...
  const tradeFees = trades.reduce((sum, t) => sum.add(new Decimal(t.fee || 0)), new Decimal(0))
  ...
  // Calculate with COP
  const latestFxRate = fxRates.length > 0 ? new Decimal(fxRates[0].rate) : null

  const depositsInCOP = cashFlows
    .filter((cf) => cf.type === "deposit")
    .reduce((sum, cf) => {
      if (cf.currency === "COP") {
