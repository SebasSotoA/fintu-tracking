...
type CashBreakdown struct {
	...
	TransferFeesUSD  string `json:"transfer_fees_usd"`
...
func (s *AnalyticsService) GetCashBreakdown(ctx context.Context, userID string) (CashBreakdown, error) {
	...
	var deposits, withdrawals, fees string
	err := s.pool.QueryRow(ctx, `
		SELECT
			...
			COALESCE(SUM(CASE WHEN type = 'fee' AND related_trade_id IS NULL THEN usd_amount ELSE 0 END), 0)
		FROM cash_flows WHERE user_id = $1
	`, userID).Scan(&deposits, &withdrawals, &fees)
	if err != nil {
		return out, err
	}
