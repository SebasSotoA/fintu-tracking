...
type CashBreakdown struct {
	...
	CashAdjustmentsUSD  string `json:"cash_adjustments_usd"`
...
func (s *AnalyticsService) GetCashBreakdown(ctx context.Context, userID string) (CashBreakdown, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT
			...
			COALESCE(SUM(CASE WHEN type = 'cash_adjustment' THEN usd_amount ELSE 0 END), 0),
	...
	err = s.pool.QueryRow(ctx, `
		SELECT
			COALESCE(SUM(CASE WHEN side = 'buy' AND COALESCE(is_opening_position, false) = false THEN quantity * price + COALESCE(total_fees, 0) ELSE 0 END), 0),
