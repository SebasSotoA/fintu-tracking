...
func (s *AnalyticsService) CalculateReturnAttribution(ctx context.Context, userID string) (models.ReturnAttribution, error) {
	...
	rows, err := s.pool.Query(ctx, `
		SELECT 
			...
			SUM(CASE WHEN t.side = 'buy' THEN (t.quantity * t.price + t.total_fees) ELSE 0 END) as total_cost,
...
func (s *AnalyticsService) GetNetWorthSummary(ctx context.Context, userID string) (models.NetWorthSummary, error) {
	...
	rows, err := s.pool.Query(ctx, `
		SELECT 
			...
			t.asset_type,
			SUM(CASE WHEN t.side = 'buy' THEN t.quantity ELSE -t.quantity END) as net_quantity,
			SUM(CASE WHEN t.side = 'buy' THEN (t.quantity * t.price + t.total_fees) ELSE -(t.quantity * t.price - t.total_fees) END) as cost_basis,
			SUM(CASE WHEN t.side = 'buy' THEN t.total_fees ELSE 0 END) as total_fees,
			mp.price as current_price
		FROM trades t
		LEFT JOIN market_prices mp ON t.ticker = mp.ticker
		WHERE t.user_id = $1
	...
	s.pool.QueryRow(ctx, `
		SELECT COALESCE(SUM(
			CASE 
				WHEN side = 'buy' THEN (quantity * price + total_fees)
				WHEN side = 'sell' THEN -(quantity * price - total_fees)
				ELSE 0
			END
		), 0)
		FROM trades
